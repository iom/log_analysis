...
combined_log_data_clean <- combined_log_data |> 
  ...
  dplyr::mutate(
    ...
    device_type = case_when(
          str_detect(user_agent, regex("mobile|iphone|android",
                                              ignore_case = TRUE)) ~ "mobile", 
          str_detect(user_agent, regex("robot|google|python",
                                              ignore_case = TRUE)) ~ "robot", 
          TRUE ~ "desktop"),
    ...
    referrer_type = case_when(
          str_detect(referer, regex("google|bing|search|yandex",
                                              ignore_case = TRUE)) ~ "search",
          str_detect(referer, regex("facebook|twitter",
                                              ignore_case = TRUE)) ~ "social",
          str_detect(referer, regex("iom|migration",
                                              ignore_case = TRUE)) ~ "iom",
          is.na(referer) ~ "direct",
          TRUE ~ "other"
        )) |>
  
  dplyr::mutate(referer_site = str_extract(referer, "(?<=https?://)([^/]+)")) |>
  ## Separating the content of the request..
  ...
  ## For clustering we need to reduce the number of class - so we lump them
  dplyr::mutate(iplocation_country2 = fct_lump_prop(iplocation_country ,
                                    prop = 0.01 , 
                                    other_level = "Other lumped (<10% total)" )) |>
  dplyr::mutate(host2 = fct_lump_prop(host ,
                                    prop = 0.01 , 
                                    other_level = "Other lumped (<10% total)" )) |>
  dplyr::mutate(referer_site2 = fct_lump_prop(referer_site ,
                                    prop = 0.01 , 
                                    other_level = "Other lumped (<10% total)" )) 

 # Anything we can clean for PID
# levels(as.factor(df$pid))

# table(combined_log_data_clean$referer_site, useNA = "ifany")
# table(combined_log_data_clean$referer_site2, useNA = "ifany")
# table(combined_log_data_clean$host, useNA = "ifany")
# table(combined_log_data_clean$host2, useNA = "ifany")
# table(combined_log_data_clean$iplocation_country, useNA = "ifany")
# table(combined_log_data_clean$iplocation_country2, useNA = "ifany")
# table(combined_log_data_clean$referrer_type, useNA = "ifany")
# table(combined_log_data_clean$device_type, useNA = "ifany")
# table(combined_log_data_clean$status_label, useNA = "ifany")

# Assuming your data frame is named 'df' - a few more filters to get something
# we can analyse
df <- combined_log_data_clean |>
      # dplyr::slice_head(n = 1000) 
      # remove all rows with size or request time over a certain threshold
...
# table(df$referer_site, useNA = "ifany")
# table(df$referrer_type, useNA = "ifany")
# table(df$referer_site2, useNA = "ifany")
# table(df$host, useNA = "ifany")
# table(df$host2, useNA = "ifany")
# table(df$iplocation_country, useNA = "ifany")
# table(df$iplocation_country2, useNA = "ifany")
# table(df$device_type, useNA = "ifany")
# table(df$status_label, useNA = "ifany")
...
mygg <- ggplot(df |> dplyr::filter( status != "200"),
  ...
  geom_bar( fill= iomthemes::iom_pal(5, "pal_blue")[1], ## IOM blue
            color = "white") +
  coord_flip()+
  labs(title="Host", x="Status Code", y="Count") +
  iomthemes::theme_iom(  font_size = 18,  grid = "X")
...
log_sessions <- df |> 
  ...
  dplyr::group_by(ip) |>
  dplyr::mutate(time_diff = as.numeric(difftime(timestamp, lag(timestamp), units = "secs"))) |> 
  dplyr::mutate(new_session = ifelse(is.na(time_diff) | time_diff > 1800, TRUE, FALSE)) |> 
  dplyr::mutate(session_id = cumsum(new_session)) |> 
  ungroup()

## levels(as.factor(log_sessions$referer))
```
...
- Content Preferences: Most-visited pages and site.
...
user_features <- log_sessions |>
  dplyr::group_by(ip, session_id, iplocation_continent , iplocation_country2) |>
  dplyr::summarise(
    session_start = min(timestamp, na.rm = TRUE),
    session_end = max(timestamp, na.rm = TRUE),
    session_duration = as.numeric(difftime(session_end, session_start, units = "secs")),
    pages_visited = dplyr::n(),
    unique_pages = dplyr::n_distinct(request_resource),
  #  top_page = request_resource[which.max(tabulate(match(request_resource, unique(request_resource))))],
    unique_site = dplyr::n_distinct(host2),
    top_site = host2[which.max(tabulate(match(host2, unique(host2))))],
    unique_referer = dplyr::n_distinct(referer_site2),
    top_referer = referer_site2[which.max(tabulate(match(referer_site2, unique(referer_site2))))],
    top_device_type = device_type[which.max(tabulate(match(device_type, unique(device_type))))],
    top_referrer_type = referrer_type[which.max(tabulate(match(referrer_type, unique(referrer_type))))]
...
mygg <- user_features   |> 
  ggplot( 
   aes(x = session_duration) ) + 
   geom_histogram(#bins = 5 ,
                  alpha = 0.7, 
                 fill= iomthemes::iom_pal(5, "pal_blue")[1], ## IOM blue
                 color = "white") +
  labs(title="Distribution of Session Duration", x=" ", y=" ") +
  iomthemes::theme_iom(  font_size = 18,  grid = "X")  
...
mygg <- user_features   |> 
  ggplot( 
   ...
   geom_histogram(#bins = 5 ,
                 ...
                 fill= iomthemes::iom_pal(5, "pal_blue")[1], ## IOM blue
                 color = "white") +
  labs(title="Distribution of pages visited", x=" ", y=" ") +
  iomthemes::theme_iom(  font_size = 18,  grid = "X")  
...
```{r usersessionsample, include=FALSE,  message = FALSE, warning = FALSE}
# dput(names(user_features ))
user_features.sel  <- user_features |>
  dplyr::select(session_duration, pages_visited, unique_pages, unique_site, unique_referer, 
         iplocation_country2, top_site, top_device_type, top_referrer_type) |>
  dplyr::mutate_if(is.character, as.factor) |>
  dplyr::mutate( session_duration = dplyr::if_else(session_duration==0,
                                                   1,session_duration)) |>
 
 # dplyr::filter( !(is.na(iplocation_continent)) ) |>
  dplyr::filter( !(is.na(iplocation_country2)) ) 

# Checking for missing values
#colSums(is.na(user_features.sel))
  
## sample to be able to run clustering...
data.sample <- user_features.sel |>
               dplyr::slice_sample(n = 10000) |>
  ## convert to factor
  dplyr::mutate(
    iplocation_country2 = as.factor(iplocation_country2),
    top_site = as.factor(top_site),
    top_device_type = as.factor(top_device_type),
    top_referrer_type = as.factor(top_referrer_type)
  )
rownames(data.sample) <- NULL
# ## Check for Duplicate Row Names
# if(any(duplicated(rownames(data.sample)))){
#   rownames(data.sample) <- make.unique(rownames(data.sample))}
# 
dplyr::glimpse(user_features.sel)
...
# numerical_data <- data.sample |>
#   dplyr::select(session_duration, pages_visited, unique_pages, unique_site, unique_referer) |>
#   scale()
# 
# # Perform Principal Component Analysis (PCA)
# pca_result <- PCA(numerical_data, graph = FALSE)
# 
# # Visualizing PCA
# fviz_eig(pca_result)
# 
# # Hierarchical Clustering on Principal Components (HCPC)
# hcpc_result <- HCPC(pca_result,
#                     nb.clust = -1, 
#                     min = 3, 
#                     max = 4, 
#                     graph = FALSE, 
...
# fviz_cluster(hcpc_result, geom = "point", ellipse.type = "convex", ggtheme = theme_minimal())
# 
# # Assign clusters to the original dataset
# data.sample$cluster <- hcpc_result$data.clust$clust
# 
# # Persona Creation - Profiling Users
# persona_profiles <- data.sample |>
#   group_by(cluster) %>%
#   summarise(
#     avg_session_duration = mean(session_duration, na.rm = TRUE),
#     avg_pages_visited = mean(pages_visited, na.rm = TRUE),
#     common_device = names(sort(table(top_device_type), decreasing = TRUE)[1]),
#     common_referrer = names(sort(table(top_referrer_type), decreasing = TRUE)[1])
#   )
# 
...
<!--  *  __Composition of the 2 axis__: Describe the component of each axis -->
<!--  *  __Categories Representation__: This allow to visualize how close different variables are -->
...
<!--  * _Cla/Mod_ is the pourcentage of records who have that modality and who are in that the profile  -->

<!--  * _Mod/Cla_ is the pourcentage of records in that profile who have that modality  -->

<!--  * _Global_ is the global pourcentage of all records who have that modality -->
...
```{r mca, include=FALSE,  message = FALSE, warning = FALSE}


#library(FactoMineR) ## Multiple correspondence analysis and classification
#library(factoextra)
# Multiple correspondence analysis
data.mca <- FactoMineR::MCA(data.sample,
                            quali.sup = which(sapply(data.sample, is.factor)),
                            #quali.sup = 6:9,
                            graph=FALSE)
```

```{r famd, include=FALSE,  message = FALSE, warning = FALSE}

# Factor Analysis of Mixed Data
famd_result <- FactoMineR::FAMD (data.sample,
                            graph=FALSE) 
# Extract FAMD principal components
#dataset_for_clustering <- famd_result$ind$coord
dataset_for_clustering <- as.data.frame(famd_result$ind$coord)

# Use the first 5 dimensions to get more variance
dataset_for_clustering <- as.data.frame(famd_result$ind$coord[, 1:5])


# Perform Hierarchical Clustering on Principal Components (HCPC)
hcpc_result <- FactoMineR::HCPC(dataset_for_clustering, nb.clust = -1, min = 3, max = 4, graph = FALSE, consol = TRUE, kk = 1000)
...
# Assign clusters to the original dataset
data.sample$cluster.famd <- hcpc_result$data.clust$clust
...
persona_profiles <- data.sample %>%
  ...
  summarise(
    ...
    common_device = names(sort(table(top_device_type), decreasing = TRUE)[1]),
    common_referrer = names(sort(table(top_referrer_type), decreasing = TRUE)[1]),
    common_country = names(sort(table(iplocation_country2), decreasing = TRUE)[1]),
    common_site = names(sort(table(top_site), decreasing = TRUE)[1])
  )



# 2. Try K-Means Instead of HCPC
# 
# Since the dataset is large, k-means may work better than hierarchical clustering:
# This method is faster and may yield better separation.
set.seed(123)  # For reproducibility
kmeans_result <- kmeans(dataset_for_clustering, centers = 4, nstart = 25)
data.sample$clusterkmean <- kmeans_result$cluster

# Visualize clusters
fviz_cluster(list(data = dataset_for_clustering, cluster = kmeans_result$cluster), 
             geom = "point", ellipse.type = "convex", ggtheme = theme_minimal())



# 3. Use Gaussian Mixture Models (GMM) for Probabilistic Clustering

#Unlike k-means, which assumes clusters are spherical, GMM allows for elliptical clusters:

# GMM assigns probabilities to each cluster, making it more flexible.
library(mclust)
gmm_result <- mclust::Mclust(dataset_for_clustering)
data$cluster <- gmm_result$classification

fviz_cluster(list(data = dataset_for_clustering, cluster = gmm_result$classification), 
             geom = "point", ellipse.type = "convex", ggtheme = theme_minimal())
...
# 
# You could create new variables:
# 
#     Interaction terms: session_duration * pages_visited
# 
#     Log transformations: log(session_duration + 1)
# 
#     Ratios: unique_pages / pages_visited

```

```{r layout='Title and Content', ph=officer::ph_location_type(type="body")}
mygg <- factoextra::fviz_mca_ind(data.mca, 
                    col.ind = iomthemes::iom_pal(5, "pal_blue")[1], ## IOM blue "blue", 
                    label="none",
                    addEllipses = TRUE,
                    ellipse.level = 0.95, 
                    jitter = list(what= "point")) +
  labs(title = "Geometric representation of Individuals in a cloud of points" ) +
  iomthemes::theme_iom(  font_size = 18) 
...
## Composition of the first axis

```{r layout='Title and Content', ph=officer::ph_location_type(type="body")}
## Contribution of top 10 variables
mygg <- factoextra::fviz_contrib(data.mca, choice = "var", axes = 1, top = 15) +
...
## Composition of the second axis

```{r layout='Title and Content', ph=officer::ph_location_type(type="body")}
## Contribution of top 10 variables
mygg <- factoextra::fviz_contrib(data.mca, choice = "var", axes = 2, top = 15) +
...
```{r layout='Title and Content', ph=officer::ph_location_type(type="body")}
mygg <- factoextra::fviz_mca_var(data.mca, 
                                 labelsize = 3, 
                                 pointsize = 0,
...
```{r clusterh, include=FALSE,  message = FALSE, warning = FALSE}
...
data.mca.hcpc <- FactoMineR::HCPC(data.mca, 
                      nb.clust = -1,   # Automatic selection of the optimal number of clusters
                      min = 3,         # Minimum number of clusters to consider
                      max = 4,         # Maximum number of clusters to consider
                      graph = FALSE,   # Disable graphical output
                      order = FALSE,   # Do not reorder the data according to clustering results
                      consol = TRUE,   # Enable consolidation of clustering results (improves stability)
                      kk = 1000)       # Number of points used for hierarchical clustering tree representation

# kable(data.mca.hcpc$desc.var$test.chi2,
#       digits = 4,
#       caption = "Ordered importance of variable contributions to profiles")
...
mygg <- factoextra::fviz_dend(data.mca.hcpc,
                              cex = 0.5, k = 4,  
...
```{r layout='Title and Content', ph=officer::ph_location_type(type="body")}
mygg <- factoextra::fviz_cluster(data.mca.hcpc, ellipse.type = "convex") +
  iomthemes::theme_iom(  font_size = 18)
...
#library(Matrix)
#use sparse matrix to store only no zero value
clustering_data <- Matrix::sparse.model.matrix(~ . -1, data = clustering_data) 
...
---
##  Website Restructuring Insights  


```{r layout='Title and Content', ph=officer::ph_location_type(type="body")}

###  Content Popularity Analysis  ######
page_analysis <- log_sessions |>
...
```{r clickstream, include=FALSE,  message = FALSE, warning = FALSE}
sessions <- log_sessions |>
  dplyr::group_by(session_id) |>
  dplyr::summarise(path = paste(request_resource, collapse = " > "))
#summary(nchar(sessions$path))
## Filter Short Sessions If your sessions are too short, remove sessions with fewer than 2 clicks:

sessions <- sessions[nchar(sessions$path) > 3, ]
library(clickstream)
# Convert sessions to clickstreams
clickstreams <- clickstream::as.clickstreams(sessions$path)
# Build Markov chain model
mc <- clickstream::fitMarkovChain(clickstreams, order = 1)
