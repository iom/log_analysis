...
combined_log_data_clean <- combined_log_data |> 
  ...
  dplyr::mutate(
    ...
    referrer_type = case_when(
          ...
          str_detect(referer, "iom.int") ~ "iom",
...
user_features <- log_sessions |>
  ...
  #  top_page = request_resource[which.max(tabulate(match(request_resource, unique(request_resource))))],
    ...
    top_site = referer_site2[which.max(tabulate(match(referer_site2, unique(referer_site2))))],
...
user_features.sel  <- user_features |>
  dplyr::select(iplocation_continent, iplocation_country2,
  session_duration, pages_visited,
  unique_site, top_site, unique_referer, top_device_type,
  top_referrer_type)
  

data.sample <- user_features.sel[sample(1:nrow(user_features.sel), 10000,replace=FALSE), ]
...
mygg <- factoextra::fviz_mca_ind(data.mca, 
                    ...
                    ellipse.level = 0.95, jitter = list(what= "point")) +
...
```{r layout='Title and Content', ph=officer::ph_location_type(type="body")}
## Contribution of top 10 variables
mygg <- factoextra::fviz_contrib(data.mca, choice = "var", axes = 2, top = 15) + theme_minimal() +
